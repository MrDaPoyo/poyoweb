...
app.post("/auth/register", async (req, res) => {
  ...
  if (!username || !email || !password) {
    ...
    try {
      ...
      if (result.success) {
        fs.mkdir(path.join(__dirname, "websites/users", username), {
          recursive: true,
        });
        res.status(201).json({
        ...
        res
          .status(400)
          .json({ error: result.message, success: result.success });
      }
    } catch (error) {
...
const storage = multer.diskStorage({
  destination: async function (req, file, cb) {
    ...
    try {
      ...
      if (!verifyFile.checkFileName(file.originalname)) {
        req.res
          .status(400)
        ...
        fs.ensureDirSync(
          path.join(__dirname, "websites/users", await username, sanitizedDir)
        );
        req.body.file = file;
        req.file = file;
        cb(
...
app.post("/file/upload", upload.single("file"), async (req, res) => {
  ...
  if (file && file.originalname && file.size) {
    ...
    userDir = path.resolve(
      path.join(__dirname, "websites/users", await user.username)
    );

    if (!resolvedPath.startsWith(userDir)) {
      return res
        .status(400)
        .json({ error: "Invalid file path", success: false });
    }

    const fileSize = (await fs.stat(fullPath)).size;
    var totalSize =
      (await db.getTotalSizeByWebsiteName(await user.username)) + fileSize;
    ...
    if ((await totalSize) < (process.env.USER_MAX_SIZE || 524288000)) {
      ...
      res
        .status(400)
        .json({ error: "Not enough space for file", success: false });
      return;
    }
  }
  res.status(400).json({
    error:
      "No file uploaded, could be due to missing fields or a suspicious name",
    success: false,
  });
});
...
app.post("/file/renameByPath", async (req, res) => {
  ...
  if (!(await user)) {
    ...
    db.insertFileInfo(db.getFileIDByPath(filePath), {
      fileLocation: newFilePath,
      fileName: newName,
      fileFullPath: path.join("websites/users", newFilePath),
      userID: await user.id,
    });
    fs.rename(
      path.join("websites/users", filePath),
      path.join("websites/users", newFilePath)
    );
    return res
      .status(200)
...
app.post("/file/removeByPath", async (req, res) => {
  ...
  try {
    ...
    var totalSize =
      (await db.getTotalSizeByWebsiteName(
        await db.findUserById(user.id).username
      )) - (await fileSize);
    db.setTotalSizeByWebsiteName(
      await db.findUserById(user.id).username,
      await totalSize
    );
  ...
  if (!user) {
    ...
    if (await username) {
      ...
      try {
        ...
        const files = await dirWalker(
          path.join(__dirname, "websites/users", await username),
          directory
        );
